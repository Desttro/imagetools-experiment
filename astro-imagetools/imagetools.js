...
export const createImageTools = () => {
	return {
		...
		getImageByAsset(asset) {
			if (!this.originalImages.has(asset)) this.originalImages.set(asset, loadImage(asset.pathname))
		...
		getTransformedImageByAsset(asset) {
			if (!this.modifiedImages.has(asset)) {
				...
				this.modifiedImages.set(asset, applyTransforms(transforms, image.clone(), true))
			}

			return this.modifiedImages.get(asset)
		},
		isImageExtension: RegExp.prototype.test.bind(/^(gif|jpeg|jpg|png|svg|webp)$/i),
